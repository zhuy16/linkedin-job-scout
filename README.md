...
Your credentials, resume, and job history never leave your machine. → [How it works & full setup guide](docs/SETUP.md)
...
cp private/profile_summary.example.txt \
   private/profile_summary.txt                # write your background here
```

You need: **Google Chrome**, an **Anthropic API key**, and a **Gmail App Password**.  
See [docs/SETUP.md](docs/SETUP.md) for step-by-step instructions on all three.
...
```bash
python run.py                  # fetch, score, send email if jobs qualify
python run.py --headless       # same, but no browser window (for automation)
python run.py --dry-run        # fetch + score, print results, skip email
python run.py --days N         # search jobs posted in the last N days (default: from config)
```

| Flag | Effect |
|------|--------|
| *(none)* | Full run — Chrome window visible, email sent for qualifying jobs |
| `--headless` | Invisible browser, good for background / scheduled runs |
| `--dry-run` | Scores jobs and prints results to stdout; never sends email |
| `--days N` | Overrides `days_back` in `config/search.yaml` for this run only |

**Test scoring without LinkedIn:**
```bash
python scripts/test_score.py   # scores a sample job against your profile, no browser needed
```
...
Edit [`config/search.yaml`](config/search.yaml) — no secrets, safe to commit:

| Key | Default | What it does |
|-----|---------|-------------|
| `query` | `("Bioinformatics" OR …)` | Boolean keyword query sent to LinkedIn |
| `skip_keywords` | `[intern, tutor, …]` | Drop any job whose title/description contains these |
| `fit_threshold` | `60` | Only email jobs with LLM score ≥ this (0–100) |
| `days_back` | `3` | Search jobs posted in the last N days |
| `max_jobs` | `25` | Cap on jobs scored per run (controls API cost) |
| `llm_model` | `claude-sonnet-4-5` | Anthropic model used for scoring |

---

## 🗓 Automated scheduling (macOS)

Runs silently at 8:00 AM every day — no Chrome window, no interruption:

```bash
cp scheduler/com.jobscout.daily.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.jobscout.daily.plist
launchctl list | grep jobscout   # verify it registered
```

Change the time in [scheduler/com.jobscout.daily.plist](scheduler/com.jobscout.daily.plist) (`Hour` / `Minute` keys). Logs go to `data/launchd.log`.
